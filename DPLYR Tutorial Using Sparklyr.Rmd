...
flights[flights$Year==2011 & flights$DepTime==1400, ]

# dplyr approach
# note: you can use comma or ampersand to represent AND condition
filter(flights, Year==2011, DepTime==1400)
```
```{r results='hide'}
sflght <- scflights #copy the dataframe to make sprk dataframe accessible
colnames(sflght)
sflght$UniqueCarrier #You will get NULL

```
```{r}
#Run the spark dtaframe on dplyr
filter(sflght, Year==2011, DepTime==1400)

```
```{r}
#You can exclude a carrier e.g continental
filter(flights, Year==2011, DepTime==1400, UniqueCarrier != "CO")
```
```{r}
#You can exclude a carrier e.g continental
#filter(flights, Year==2011, DepTime==1400, UniqueCarrier != "CO") %>% show_query()
```
```{r}
filter(sflght, Year==2011, DepTime==1400, UniqueCarrier != "CO")
```

```{r}
# note: you can use comma or ampersand to represent AND condition
# use pipe for OR condition
filter(sflght, UniqueCarrier=="AA" | UniqueCarrier=="UA")
```
...
```{r}
# dplyr approach
select(sflght, DepTime, ArrTime, FlightNum)

```
```{r}
sflght %>% select(DepTime, ArrTime, FlightNum)
```
```{r}
# use colon to select multiple contiguous columns, 
sflght %>% select(Year:FlightNum) 

```
```{r}
# and use `contains` to match columns by name. note: `starts_with`, `ends_with`, and `matches` (for regular expressions) can also be used to match columns by name
select(sflght, Year:DayofMonth, contains("Taxi"), contains("Delay"))
```
...
```{r results='hide'}
sflght %>% select(Year:DayofMonth, starts_with("Taxi"), ends_with("Delay"))
#comapare it with
#sflght %>% select(Year:DayofMonth, starts_with("Taxi"), starts_with("Delay")))

```
```{r results='hide'}
# nesting method to select UniqueCarrier and DepDelay columns and filter for delays over 60 minutes
filter(select(sflght, UniqueCarrier, DepDelay), DepDelay > 60)
```
...
sflght  <- sflght  %>% mutate(Speed = Distance/AirTime*60)
```

```{r}
filter(sflght, Origin != "IAH", Distance > 1000, Speed >= 400)
```
...
sflght %>% sample_frac(0.25, replace=TRUE)
...
conX <- conn$con
```
```{sql connection=conX, output.var="storm_preview"}
SELECT * FROM storms LIMIT 12;
```
```{r}
storm_preview
```
```{r}
#Send this last query to a dataframe
tableNM <- tbl(conn, sql("SELECT * FROM storms LIMIT 6"))
colnames(tableNM)
```
```{sql connection=conX, output.var="storm_preview"}
SELECT name, status, month, day  FROM storms;
```
```{r}
storm_preview
```
```{sql connection=conX, output.var="storm_preview"}
SELECT distinct name, status, month, day  FROM storms;
```
```{r}
storm_preview
```
```{sql connection=conX, output.var="storm_preview"}
SELECT name, status, month, day FROM storms WHERE status = "tropical storm";
```
```{r}
storm_preview
```
...
```{sql connection=conX, output.var="storm_preview"}
SELECT name, status, month, day FROM storms WHERE hour <= 6;
```
```{r}
storm_preview
```
...
```{sql connection=conX, output.var="storm_preview"}
SELECT name, status,year, month, day FROM storms WHERE hour >= 6 AND lat >= 20 ORDER BY year DESC;
```
```{r}
storm_preview
#Remember this would only let you show your query. In order to create a dataframe you have to follow the syntax below.
```

```{r}
system('banner Relax Kalyan') #Runs on linux
```
```{r}
#Send the last query to a dataframe
tableDF <- tbl(conn, sql("SELECT name, status, month, day FROM storms WHERE hour <= 6"))
print(tableDF)
```

```{sql connection=conX, output.var="storm_preview"}
SELECT name, status,year, month, day , wind, pressure, ts_diameter, hu_diameter FROM storms WHERE hour >= 6 AND lat >= 20 ORDER BY  ts_diameter DESC;
```
```{r}
storm_preview
```
```{sql connection=conX, output.var="storm_preview"}
SELECT name, status,year, month, day , wind, pressure, ts_diameter, hu_diameter FROM storms WHERE hour >= 6 AND lat >= 20 AND ts_diameter != "" ORDER BY  ts_diameter DESC;
/*SELECT name, status,year, month, day , wind, pressure, ts_diameter, hu_diameter FROM storms WHERE hour >= 6 AND ts_diameter != "" ORDER BY  ts_diameter DESC;*/
```
```{r}
storm_preview
```